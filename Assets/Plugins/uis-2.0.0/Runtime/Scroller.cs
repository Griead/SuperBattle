...
namespace UIS {
    ...
    public class Scroller : MonoBehaviour, IDropHandler {
        ...
        /// 获取UI数据委托
        /// </summary>
        private CustomDelegateOut<int, UIData> _getUIDataDelegate;
        ...
        public void InitData(int count, CustomDelegateOut<int, UIData> getUIDataDelegate = null, bool isOtherSide = false) {
            if (count <= 0) {
            ...
            _isInited = true;
            _getUIDataDelegate = getUIDataDelegate;
            if (Type == 0) {
        ...
        void CreateViewsVertical(bool isForceCreate = false) {
            ...
            if (childs > 0 && !isForceCreate) {
                ...
                for (var i = 0; i < fillCount; i++) {
                    // 区分下是否是框架实例化
                    if (_getUIDataDelegate == null) {
                        clone = UnityEngine.Object.Instantiate(Prefab, Vector3.zero, Quaternion.identity);
                    } else {
        ...
        private GameObject Instantiate(GameObject prefab, int index) {

            var task = UIFrame.Instantiate(prefab, null, _getUIDataDelegate(index));
